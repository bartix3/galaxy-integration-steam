...
class PluginStatus:
    ...
    """
    state: PluginState = PluginState.UNALLOCATED
    critical_GOG_Exception: Optional[ApplicationError] = None

    @classmethod
    def set_critical_error(cls, err: ApplicationError):
        cls._state = PluginState.CRITICAL_ERROR
        cls.critical_GOG_Exception = err

    @classmethod
    def recover_critical_error(cls, new_state: PluginState) -> bool:
        if new_state != PluginState.CRITICAL_ERROR:
            cls.state = new_state
            cls.critical_GOG_Exception = None
            return True
        else:
            return False

    @classmethod
    def update_plugin_state(cls, new_state: PluginState) -> bool:
        if cls.state != PluginState.CRITICAL_ERROR:
            cls.state = new_state
            return True
        else:
            return False